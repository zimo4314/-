...
const Archive3D = {
  init: function (canvasContainer, onJump) {
    ...
    this.Colors = {
      ...
      gold: 0xffd700,
      black: 0x000000,
      wall: 0xeeeeee, // 调整墙壁颜色为更柔和的米白/浅灰
      highlight: 0xffff00, // 高亮颜色
      wallUpper: 0xf5f5f5,   // 上半部分墙面（米白）
      wallLower: 0xdcdcdc,   // 下半部分墙面（浅灰）
      wainscot: 0x010101,    // 腰线/护墙板顶部装饰色（深蓝灰）
      ceiling: 0xffffee,     // 天花板
      ceilingLight: 0xffffee, // 灯光颜色
      baseboard: 0x1a1a1a    // 踢脚线
    };
  ...
  // 绘制墙壁 (升级版：封闭房间 + 踢脚线 + 天花板)
// 绘制装饰性墙壁和天花板（包含物理碰撞优化）
  drawWalls: function() {
    // === 1. 参数设置 ===
    const wallHeight = 300;     // 房高
    const wallThickness = 40;   // 【关键】增加墙厚度，防止穿模
    const halfSize = ROOM_SIZE / 2;

    // 装饰参数
    const wainscotHeight = 100; // 护墙板高度（下半截墙）
    const trimSize = 4;         // 装饰条突出的尺寸

    // === 2. 材质准备 ===
    // 上半墙面
    const upperWallMat = new THREE.MeshPhongMaterial({
      color: 0xf0f0f0, // 稍微带点灰的白
    });
    // 下半墙面 (护墙板)
    const lowerWallMat = new THREE.MeshPhongMaterial({
      color: 0xe0e6ed, // 淡蓝色调的灰
    });
    // 腰线 (深色分割线)
    const waistMat = new THREE.MeshPhongMaterial({ color: 0x1e3a8a });
    // 踢脚线
    const baseMat = new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.5 });
    // 顶角线
    const crownMat = new THREE.MeshStandardMaterial({ color: 0xffffff });

    // === 3. 核心造墙函数 ===
    // width/depth: 墙的总长宽
    // x/z: 中心位置
    // hasDoor: 是否有门洞逻辑
    const createDecoratedWall = (width, depth, x, z, hasDoor = false) => {
      const wallGroup = new THREE.Group();

      // 判断墙的朝向 (横向还是纵向)
      const isHorizontal = width > depth;
      // 实际厚度方向的尺寸
      const thick = isHorizontal ? depth : width;
      const longSide = isHorizontal ? width : depth;

      // --- A. 物理防穿墙核心 (隐形或内部的主墙体) ---
      // 我们用一个稍微厚一点的Box作为物理碰撞的主体
      const mainGeo = new THREE.BoxGeometry(width, wallHeight, depth);
      const mainWall = new THREE.Mesh(mainGeo, upperWallMat);
      mainWall.position.y = wallHeight / 2 + SCENE_Y_OFFSET;
      // 开启投影
      mainWall.castShadow = true;
      mainWall.receiveShadow = true;
      wallGroup.add(mainWall);

      // --- B. 装饰细节 (如果是门洞墙，逻辑会复杂，这里简化处理只给普通墙加装饰) ---
      if (!hasDoor) {
        // 1. 下半截颜色覆盖 (护墙板)
        // 为了不穿插闪烁(z-fighting)，稍微比主墙厚一点点 (0.2)
        const lowerGeo = new THREE.BoxGeometry(
          isHorizontal ? width + 0.2 : thick + 2,
          wainscotHeight,
          isHorizontal ? thick + 2 : depth + 0.2
        );
        const lowerWall = new THREE.Mesh(lowerGeo, lowerWallMat);
        lowerWall.position.y = wainscotHeight / 2 + SCENE_Y_OFFSET;
        lowerWall.receiveShadow = true;
        wallGroup.add(lowerWall);

        // 2. 腰线 (Wainscot cap) - 分割上下墙面
        const waistGeo = new THREE.BoxGeometry(
          isHorizontal ? width + 0.4 : thick + 4,
          5, // 腰线高度
          isHorizontal ? thick + 4 : depth + 0.4
        );
        const waist = new THREE.Mesh(waistGeo, waistMat);
        waist.position.y = wainscotHeight + 2.5 + SCENE_Y_OFFSET;
        wallGroup.add(waist);

        // 3. 踢脚线 (Baseboard) - 最底部
        const baseGeo = new THREE.BoxGeometry(
          isHorizontal ? width + 0.6 : thick + 6,
          12,
          isHorizontal ? thick + 6 : depth + 0.6
        );
        const base = new THREE.Mesh(baseGeo, baseMat);
        base.position.y = 6 + SCENE_Y_OFFSET;
        wallGroup.add(base);

        // 4. 顶角线 (Crown Molding) - 最顶部
        const crownGeo = new THREE.BoxGeometry(
          isHorizontal ? width + 1 : thick + 4,
          10,
          isHorizontal ? thick + 4 : depth + 1
        );
        const crown = new THREE.Mesh(crownGeo, crownMat);
        crown.position.y = wallHeight - 5 + SCENE_Y_OFFSET;
        wallGroup.add(crown);
      }

      wallGroup.position.set(x, 0, z);
      this.group.add(wallGroup); // 加入物理组
    };

    // === 4. 构建四周 ===

    // 后墙
    createDecoratedWall(ROOM_SIZE, wallThickness, 0, -halfSize - wallThickness/2);
    // 左墙
    createDecoratedWall(wallThickness, ROOM_SIZE, -halfSize - wallThickness/2, 0);
    // 右墙
    createDecoratedWall(wallThickness, ROOM_SIZE, halfSize + wallThickness/2, 0);

    // 前墙 (带门洞，拆分为左右两块)
    const frontWallLen = (ROOM_SIZE - DOOR_WIDTH) / 2;
    const frontZ = halfSize + wallThickness/2;

    // 前左
    createDecoratedWall(frontWallLen, wallThickness, -halfSize + frontWallLen/2, frontZ, true);
    // 前右
    createDecoratedWall(frontWallLen, wallThickness, halfSize - frontWallLen/2, frontZ, true);
    ...
    const doorHeader = new THREE.Mesh(
      new THREE.BoxGeometry(DOOR_WIDTH, headerH, wallThickness),
      upperWallMat
    );
    doorHeader.position.set(0, wallHeight - headerH/2 + SCENE_Y_OFFSET, frontZ);
    this.group.add(doorHeader);

    // === 5. 调用绘制天花板 ===
    this.drawCeiling(wallHeight, wallThickness);
  },

  // 绘制天花板 (带灯光装饰)
  drawCeiling: function(height, thickness) {
    // 1. 基础天花板
    const ceilMat = new THREE.MeshPhongMaterial({ color: 0xffffff, side: THREE.FrontSide });
    const ceilGeo = new THREE.BoxGeometry(ROOM_SIZE + thickness*2, 10, ROOM_SIZE + thickness*2);
    const ceiling = new THREE.Mesh(ceilGeo, ceilMat);
    ceiling.position.set(0, height + 5 + SCENE_Y_OFFSET, 0);
    // 天花板加入group，防止跳出去
    this.group.add(ceiling);

    // 2. 装饰性网格梁 (Grid Beams)
    const beamMat = new THREE.MeshPhongMaterial({ color: 0xdddddd });
    const gridSize = 4; // 4x4 的格子
    const step = ROOM_SIZE / gridSize;

    // 绘制横纵梁
    for(let i = 1; i < gridSize; i++) {
      const offset = -ROOM_SIZE/2 + i * step;

      // 横梁 X方向
      const beamX = new THREE.Mesh(
        new THREE.BoxGeometry(ROOM_SIZE, 15, 10),
        beamMat
      );
      beamX.position.set(0, height - 7.5 + SCENE_Y_OFFSET, offset);
      this.group.add(beamX);

      // 纵梁 Z方向
      const beamZ = new THREE.Mesh(
        new THREE.BoxGeometry(10, 15, ROOM_SIZE),
        beamMat
      );
      beamZ.position.set(offset, height - 7.5 + SCENE_Y_OFFSET, 0);
      this.group.add(beamZ);
    }

    // 3. 面板灯 (Panel Lights) - 发光效果
    const lightGeo = new THREE.BoxGeometry(ROOM_SIZE/gridSize - 20, 2, ROOM_SIZE/gridSize - 20);
    const lightMat = new THREE.MeshBasicMaterial({ color: 0xffffee }); // 自发光不需要光照计算

    // 在每个格子里放一个灯
    const startPos = -ROOM_SIZE/2 + step/2;
    for(let x = 0; x < gridSize; x++) {
      for(let z = 0; z < gridSize; z++) {
        // 棋盘格布局灯光，或者全铺，这里选择间隔铺设显得有设计感
        if ((x + z) % 2 === 0) {
          const lightPanel = new THREE.Mesh(lightGeo, lightMat);
          lightPanel.position.set(
            startPos + x * step,
            height - 1 + SCENE_Y_OFFSET, // 稍微贴近天花板
            startPos + z * step
          );
          this.group.add(lightPanel);

          // 可选：添加实际的点光源让灯看起来真的在发光
          // 注意：太多点光源会影响性能，酌情添加
          // const pointLight = new THREE.PointLight(0xffaa00, 0.3, 300);
          // pointLight.position.copy(lightPanel.position);
  ...
  addEvent: function(){
    ...
    document.addEventListener("mousemove", (event) => {
      ...
      if(this.ifTurn){
        ...
        _this.capsuleBodyControl.rotation.x = Math.max(
          -Math.PI / 2 + 0.8, // 最小角度：允许向上看直到接近垂直（为了看天花板）
          Math.min(Math.PI / 10, _this.capsuleBodyControl.rotation.x) // 最大角度：0，即水平视角，禁止向下看地板
        );
      }
    }, false);
  },
